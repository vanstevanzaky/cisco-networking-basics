# Labs - Packet Tracer

# Packet Tracer Labs - Index

Kumpulan 13 lab praktik dengan dokumentasi lengkap hasil pembelajaran.

## 📚 Labs

| # | Lab Name | Module | Status | Completed |
|----|----------|--------|--------|-----------|
| 1 | Configure Wireless Router and Client | 4.4.4 | ✅ | 18 Jan 2026 |
| 2 | Connect to a Web Server | 8.1.3 | ✅ | 19 Jan 2026 |
| 3 | Configure DHCP on Wireless Router | 11.2.3 | ✅ | 24 Jan 2026 |
| 4 | Examine NAT on Wireless Router | 12.2.2 | ✅ | 25 Jan 2026 |
| 5 | Identify MAC and IP Addresses | 13.1.3 | ✅ | 28 Jan 2026 |
| 6 | Observe Traffic Flow in Routed Network | 14.3.3 | ✅ | 31 Dec 2024 |
| 7 | Create a LAN | 14.3.4 | ✅ | 31 Dec 2024 |
| 8 | The Client Interaction | 16.1.5 | ✅ | 1 Feb 2026 |
| 9 | Observe Web Requests | 16.4.3 | ✅ | 2 Feb 2026 |
| 10 | Use FTP Services | 16.5.3 | ✅ | 2 Feb 2026 |
| 11 | Use Telnet and SSH | 16.6.4 | ✅ | 2 Feb 2026 |
| 12 | Use the ipconfig Command | 17.1.3 | ⏳ | - |
| 13 | Use the ping Command | 17.1.6 | ⏳ | - |

## 📝 Format Setiap Lab

Setiap folder lab berisi:
- **README.md** - Laporan lengkap dengan:
  - Objectives
  - Konfigurasi yang dilakukan
  - Hasil testing
  - Learning outcomes
  - Insights & best practices
- **screenshots/** - Folder untuk dokumentasi visual
- **.pkt file** - Packet Tracer topology file

---

**Catatan:** Update status lab setelah setiap praktik selesai.
